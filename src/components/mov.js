...
class Movie extends Component {
  ...
  poster(imdb) {
    ...
    axios
      ...
      .catch(function(error) {
        self.setState({
          posterIMG:
            'http://www.beauty-beyond.ca/_assets/img/page/800x1000placeholder.jpg'
        });
        console.log(error);
      });
  }
  ...
  componentWillMount() {
    ...
    schemaBuilder.connect().then(db => {
      ...
      db.select()
        .from(movies)
        .where(movies.movie_title.eq(this.props.match.params.movie))
        .exec()
        .then(function(rows) {
  ...
  render() {
    ...
    if (movie.budget) {
      budget = Math.abs(Number(movie.budget)) / 1.0e6 + 'M';
    }
    return (
      <div className="moviecard">
        <div className="movie-poster" style={sectionStyle} />
        <div id="movie-content">
          <div className="movie-ratings">
            <span className="title">Country : </span>
            <span className="score-out-of">{movie.country}</span>
          </div>
          <div className="movie-title">
            <a href={movie.movie_imdb_link} target="_blank">
              {movie.movie_title}
            </a>
            <span className="movie-year">{movie.title_year}</span>
          </div>
          <div className="movie-details">
            <span className="movie-rating">{movie.content_rating}</span>
            <span className="movie-duration">{movie.language}</span>
            <span className="movie-genre">{genres}</span>
          </div>
          <div className="movie-castcrew">
            <span className="title">Director</span>
            <span className="name">{movie.director_name}</span>
          </div>
          <div className="movie-castcrew">
          ...
          <div className="movie-key">
            <span className="movie-genre">{keywrds}</span>
          </div>
        </div>
      </div>
    );
