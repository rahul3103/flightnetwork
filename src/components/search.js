...
class SearchBar extends Component {
  ...
  movieSearch(term) {
    ...
    schemaBuilder.connect().then(db => {
        ...
        db.select().from(movies).where(movies.movie_title.match(term.toLowerCase())).limit(5).exec().then(rows => this.setState ({searchedMovie: rows}))
      })
  }
  ...
  render() {
    let ui = null;
    ui = 
          this.state.searchedMovie.map(movie => {
            return (<li className="result" key={movie.movie_title}> <a target="_blank" href={"movie/" + movie.movie_title}>{movie.movie_title}</a></li>);
          })
    return (
      <form>
